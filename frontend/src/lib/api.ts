...
const api = {
  ...
  getTestExecutionsBySuite: (days: number = 7) => 
    fetch(`${API_URL}/api/reports/test-executions-by-suite?days=${days}`).then(handleResponse),
  getProjectsWithBugs: () => 
    fetch(`${API_URL}/api/reports/projects-with-bugs`).then(handleResponse),
  getBugsPerTester: (days: number = 7) => 
    fetch(`${API_URL}/api/reports/bugs-per-tester?days=${days}`).then(handleResponse),
  getBugsDiscoveredLastWeek: () => 
    fetch(`${API_URL}/api/reports/bugs-discovered-last-week`).then(handleResponse),
  getUnassignedBugs: () => 
    fetch(`${API_URL}/api/reports/unassigned-bugs`).then(handleResponse),
  
  getOpenIssuesByProject: (days: number = 30) => 
    fetch(`${API_URL}/api/charts/open-issues-by-project?days=${days}`).then(handleResponse),
  getClosedIssuesByProject: (days: number = 30) => 
    fetch(`${API_URL}/api/charts/closed-issues-by-project?days=${days}`).then(handleResponse),
  getBugStatusDistribution: () => 
    fetch(`${API_URL}/api/charts/bug-status-distribution`).then(handleResponse),
  getBugSeverityDistribution: () => 
    fetch(`${API_URL}/api/charts/bug-severity-distribution`).then(handleResponse),
};
