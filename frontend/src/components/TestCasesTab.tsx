      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Add a new test case to a test suite</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value) => setFormData({ ...formData, test_suite_id: parseInt(value) })}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {testSuites.map(suite => (
                      <SelectItem key={suite.test_suite_id} value={suite.test_suite_id!.toString()}>
                        {suite.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <form onSubmit={handleExecutionSubmit} className="space-y-4">
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setExecutionForm({ ...executionForm, tester_id: parseInt(value) })}
                  >
                    <SelectTrigger>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {testCases.map(testCase => {
                const suite = testSuites.find(s => s.test_suite_id === testCase.test_suite_id);
                return (
                  <div key={testCase.test_case_id} className="border rounded-lg p-4 bg-white">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <h3 className="font-semibold">{testCase.name}</h3>
                        {testCase.description && <p className="text-sm text-gray-600 mt-1">{testCase.description}</p>}
              ...
              {testCases.length === 0 && (
                <p className="text-center text-gray-500 py-8">No test cases yet. Create your first test case!</p>
              )}
            </div>
          )}
        </CardContent>
