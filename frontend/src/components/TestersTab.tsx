...
import { useTesters, useCreateTester } from '../hooks/useQueries';
...
export default function TestersTab() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    createTester.mutate(formData, {
      onSuccess: () => {
        setFormData({ name: '', email: '', role: '', date_joined: new Date().toISOString().split('T')[0] });
      }
    });
  };
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={createTester.isPending}>
              {createTester.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="space-y-3">
              {testers.map(tester => (
                <div key={tester.tester_id} className="border rounded-lg p-4 bg-white">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      {tester.role && <p className="text-sm text-gray-500 mt-1">{tester.role}</p>}
                    </div>
              ...
              {testers.length === 0 && (
                <p className="text-center text-gray-500 py-8">No testers yet. Add your first tester!</p>
              )}
            </div>
          )}
        </CardContent>
