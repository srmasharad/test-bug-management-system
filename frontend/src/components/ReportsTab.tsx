    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Select value={report1Days.toString()} onValueChange={(value) => handleReport1DaysChange(parseInt(value))}>
              ...
              <tbody>
                {report1Data.map((row, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    ...
                    <td className="text-right p-2 font-semibold text-blue-600">{row.recent_executions}</td>
                  </tr>
                ))}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Summary of current projects under test with bug counts per sub-project
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
              ...
              <tbody>
                {report2Data.map((row, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    ...
                    <td className="text-right p-2 font-semibold">{row.total_bugs}</td>
                    <td className="text-right p-2 text-yellow-600">{row.open_bugs}</td>
                    <td className="text-right p-2 text-green-600">{row.closed_bugs}</td>
                    <td className="text-right p-2 text-red-600">{row.critical_bugs}</td>
                    <td className="text-right p-2 text-orange-600">{row.high_bugs}</td>
                    <td className="text-right p-2 text-yellow-600">{row.medium_bugs}</td>
                    <td className="text-right p-2 text-green-600">{row.low_bugs}</td>
                  </tr>
                ))}
                {report2Data.length === 0 && (
                  <tr>
                    <td colSpan={9} className="text-center p-4 text-gray-500">No data available</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Select value={report3Days.toString()} onValueChange={(value) => handleReport3DaysChange(parseInt(value))}>
              <SelectTrigger className="w-40">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
              ...
              <tbody>
                {report3Data.map((row, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    ...
                    <td className="text-right p-2 font-semibold text-blue-600">{row.bugs_assigned_period}</td>
                    <td className="text-right p-2">{row.total_bugs_assigned}</td>
                    <td className="text-right p-2 text-green-600">{row.bugs_resolved}</td>
                  </tr>
                ))}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            List of bugs discovered during the last 7 days with tester info and test case links
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
              ...
              <div key={idx} className="border rounded-lg p-3 hover:bg-gray-50">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <h4 className="font-semibold">#{bug.bug_id} - {bug.bug_name}</h4>
                    {bug.description && <p className="text-sm text-gray-600 mt-1">{bug.description}</p>}
                    <div className="flex gap-2 mt-2 flex-wrap">
                      <span className={`px-2 py-0.5 rounded text-xs ${
                        bug.status === 'Open' || bug.status === 'New' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'
                      }`}>
                        {bug.status}
                      </span>
                      <span className={`px-2 py-0.5 rounded text-xs ${
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-3 text-xs text-gray-600">
                  <div>
                    <span className="font-medium">Discovered By:</span> {bug.discovered_by_name}
                  </div>
                  <div>
                    <span className="font-medium">Email:</span> {bug.tester_email}
                  </div>
                  {bug.test_case_name && (
                    <div>
                      <span className="font-medium">Test Case:</span> {bug.test_case_name}
                    </div>
                  )}
                  <div>
                    <span className="font-medium">Project:</span> {bug.project_name}
                  </div>
                  <div>
            ...
            {report4Data.length === 0 && (
              <p className="text-center text-gray-500 py-8">No bugs discovered in the last week</p>
            )}
          </div>
        </CardContent>
      </Card>
          ...
          <div className="space-y-3">
            {report5Data.map((bug, idx) => (
              <div key={idx} className={`border rounded-lg p-3 ${
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <h4 className="font-semibold">#{bug.bug_id} - {bug.name}</h4>
                    {bug.description && <p className="text-sm text-gray-700 mt-1">{bug.description}</p>}
                    <div className="flex gap-2 mt-2 flex-wrap">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                        bug.severity === 'Critical' ? 'bg-red-200 text-red-900' :
                        bug.severity === 'High' ? 'bg-orange-200 text-orange-900' :
                        bug.severity === 'Medium' ? 'bg-yellow-200 text-yellow-900' :
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-3 text-xs text-gray-700">
                  ...
                  {bug.sub_project_name && (
                    <div>
                      <span className="font-medium">Sub-Project:</span> {bug.sub_project_name}
                    </div>
                  )}
                  {bug.environment && (
                    <div>
            ...
            {report5Data.length === 0 && (
              <p className="text-center text-gray-500 py-8">All bugs are assigned</p>
            )}
          </div>
        </CardContent>
      </Card>
