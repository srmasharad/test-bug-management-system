    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Add a new test suite to organize test cases</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {testSuites.map(suite => {
                const project = projects.find(p => p.project_id === suite.project_id);
                return (
                  <div key={suite.test_suite_id} className="border rounded-lg p-4 bg-white">
                    <h3 className="font-semibold">{suite.name}</h3>
                    {suite.description && <p className="text-sm text-gray-600 mt-1">{suite.description}</p>}
                    <div className="flex gap-4 mt-2 text-xs text-gray-500">
                      <span>Project: {project?.name}</span>
                      {suite.created_date && <span>Created: {new Date(suite.created_date).toLocaleDateString()}</span>}
                    </div>
                  </div>
                );
              })}
              {testSuites.length === 0 && (
                <p className="text-center text-gray-500 py-8">No test suites yet. Create your first test suite!</p>
              )}
            </div>
          )}
        </CardContent>
